...
type APReadings = HashMap Polar (Seq Double)

-- Make Polar hashable so we can use it as a key in APReadings
deriving instance Hashable Polar
...
insertGpsPoint essid bssid loc db =
  HashMap.insertWith (HashMap.unionWith $ HashMap.unionWith (<>)) essid .
  HashMap.singleton bssid . HashMap.singleton loc $
  Seq.singleton db
...
toHeatpoints =
  Vector.fromList .
  toList . fmap (Vector.fromList . fmap heatpoint . HashMap.toList)
  where
    heatpoint (p, db) =
